var date = [
    ...
    {active:true,name:1,month:5,day:29,title:"今天是@包头九中赤园青年马克思主义学社的生日！"},
...
for(var i = 0;i<date.length;i++){
    if(nowmonth == date[i].month && nowdate == date[i].day){
        document.getElementById('celebrates-container').innerHTML += '<div class="article-box">'+'<div class="ab-content">'+'<a href="../templates/about.html?name="'+date[i].name+' class="article-img-box">'+'<img class="lazy-image article-img" data-src="../essays_img/leninism1.jpeg" alt="" src="../essays_img/leninism1.jpeg">'+'</a>'+'<div class="article-title">'+'<a href="../templates/about.html?name="'+date[i].name+'>'+date[i].title+'</a>'+'</div>'+'<div class="article-detail-box c-666">联合斗争万岁！</div>'+'<span class="article-tail-box"><span class="article-date c-999">'+nowyear+"-"+nowmonth+"-"+nowdate+'</span>'+'<span class="article-author one-line-overflow c-999">望关Bot</span>'+'</span>'+'</div>'+'</div>'
    }
}